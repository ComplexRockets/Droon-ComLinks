...
public class EditorStructure : MonoBehaviour {
    ...
    public void CreateMesh (float radius, float depth, int resolution, float structurePos, float strucureRadius, int suportArmCount, int verticsToShow, float verticsScale, float doubleSupportArmOffset, Vector3 center, bool collider = false) {
        _radius = radius;
        _depth = depth;
        _strucureRadius = strucureRadius;
        _verticsScale = verticsScale;
        _verticsToShow = verticsToShow;
        ...
        for (int i = 0; i < suportArmCount; i++, angle += angleStep) {
            ...
            Vector3 A = new Vector3 (center.x, _focalLength, 0);
            Vector3 B = new Vector3 (pos, HeightOnParabola (pos));

            Point (_focalLength, 0, angle, Vector3.right);
            AnglePoint (_focalLength, center.x, angle, Vector3.right, B - A);
            PointOnParabola (pos, angle + angleStep * doubleSupportArmOffset / 2, B - A);
            ConnectPoints (3);
            if (doubleSupportArmOffset > 0) {
                AnglePoint (_focalLength, center.x, angle, noPoint, B - A);
                PointOnParabola (pos, angle + angleStep * -doubleSupportArmOffset / 2, B - A);
                ConnectPoints (3);
            }
            Point (_focalLength / 2, _radius, angle, Vector3.up, center.y);
            pointsToConnect--;
        }
        ...
        if (!collider) {
            ...
            CreateMesh (radius, depth, 3, structurePos, strucureRadius, suportArmCount, verticsToShow, verticsScale, doubleSupportArmOffset, center, collider = true);
        } else {
    ...
    private void OnDrawGizmos () {
        if (Application.isPlaying) {
            if (_verticsToShow > _vertices.Length) _verticsToShow = _vertices.Length;
            else if (_verticsToShow < 0) _verticsToShow = 0;

            for (int v = 0; v < _verticsToShow; v++) {
                Gizmos.DrawSphere (_vertices[v], _verticsScale);
            }

            Gizmos.DrawSphere (new Vector3 (0, _focalLength, 0), _verticsScale);
            Gizmos.DrawSphere (new Vector3 (0, _depth, 0), _verticsScale);
            Gizmos.DrawSphere (new Vector3 (), _verticsScale);
        }
    }
}