using System;
using UnityEngine;
public class EditorParabolicAntennaMesh : MonoBehaviour {
    ...
    public void CreateMesh (float r, float depth, int parabolaRes, int parabolaDivCount, float thickness, float ridgeA, float ridgeB, int ridgeDivCount, float ridgeWidth, bool straightSide, float bottomRadius, float bottomDepth, float bottomOffset,
        int structureRes, float structurePos, float strucureRadius, int suportArmCount, float doubleSupportArmOffset, Vector2 offset, bool collider = false) {
        _radius = r;
        _depth = depth;
    ...
    private void CreateParabola () {
        ...
        if (_bottomOffset != 0) {
            AddDivision (height, curentRadius, 2); // Sharp Edge
            AddDivision (-_bottomdepth - _bottomOffset, _bottomRadius, 2); //Bottom Offset
        }

        //LastVertex
        verticsIndex++;
        _vertices[_vertices.Length - 1] = new Vector3 (0, -_bottomdepth - _bottomOffset, 0);
        _uvs[_vertices.Length - 1] = new Vector4 (0, 0, 0, _bottomOffset != 0 ? subPartId * 10f + 2 + 0.1f : subPartId * 10f + 1 + 0.1f);
        lowestPoint = _vertices[_vertices.Length - 1].y;
    ...
    private void GenerateRidge (float radius, float heightOffset, int direction, out float endRadius, out float endHeight, int matId) {
        ...
        for (int div = _parabolaDivCount; div < _parabolaDivCount + _ridgeDivCount; div++) {
            ...
            if (endHeight < _ridgeEndY) { _ridgeEndY = endHeight; _ridgeEndX = radius; }
        }
        endRadius = radius;
    }
    ...
    private void CreateStructure () {
        ...
        for (int i = 0; i < _suportArmCount; i++, angle += angleStep) {
            ...
            Vector3 A = new Vector3 (_offset.y, _focalLength, 0);
            Vector3 B = new Vector3 (pos, HeightOnParabola (pos));

            Point (_focalLength + _offset.x * _radius, 0, angle, Vector3.right);
            AnglePoint (_focalLength + _offset.x * _radius, _offset.y * _radius, angle, Vector3.right, B - A);
            PointOnParabola (pos, angle + angleStep * _doubleSupportArmOffset / 2, B - A);
            ConnectPoints (3);
            if (_doubleSupportArmOffset > 0) {
                AnglePoint (_focalLength + _offset.x * _radius, _offset.y * _radius, angle, noPoint, B - A);
                PointOnParabola (pos, angle + angleStep * -_doubleSupportArmOffset / 2, B - A);
                ConnectPoints (3);
            }
        }
    ...
    private void GenerateSideStructure () {
        ...
        Vector2 BottomAnchor = new Vector2 (_bottomRadius, lowestPoint);
    }

    private void OnDrawGizmos () {
        Gizmos.DrawLine (new Vector2 (_ridgeEndX, _ridgeEndY), new Vector2 (_bottomRadius, lowestPoint));
        _ridgeEndX = 0;
        _ridgeEndY = Mathf.Infinity;
    }
    ...
    private void ApplyMesh (bool collider) {
        if (collider) {
            ...
            CreateMesh (_radius, _depth, 16, 2, _thickness, 0, 0, 0, 0, _straightSide, _bottomRadius, _bottomdepth, _bottomOffset, 3, _structurePos, _strucureRadius, _suportArmCount, _doubleSupportArmOffset, _offset, true);
        }
    }
