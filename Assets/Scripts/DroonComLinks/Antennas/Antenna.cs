    ...
    public class Antenna : IDisplayable
    {
        ...
        public IUIListItem[,] GetInfo()
        {
            if (info == null)
            {
                ...
                info[0, 0] = (IUIListItem)new UIListTextValue<string>("Type ", () => antennaType.id);
                info[0, 1] = (IUIListItem)new UIListTextValue<string>("NetworkNode ", () => node.id);
                info[0, 2] = (IUIListItem)new UIListTextValue<int>("PartID ", () => partId);
                info[0, 3] = (IUIListItem)new UIListTextValue<float>("Gain ", () => ((int)(centerFrequencyGain * 100)) / 100);
                info[0, 4] = (IUIListItem)new UIListTextValue<string>("Min Frequency ", () => frequencies[0] + " GHz");
                info[0, 5] = (IUIListItem)new UIListTextValue<string>("Center Frequency ", () => frequencies[1] + " GHz");
                info[0, 6] = (IUIListItem)new UIListTextValue<string>("Max Frequency ", () => frequencies[2] + " GHz");
                info[0, 7] = (IUIListItem)new UIListTextValue<string>("fractional Band Width ", () => fractionalBandWidth + " %");
                info[0, 8] = (IUIListItem)new UIListTextValue<string>("Center WaveLength", () => $"{waveLengths[1]:n2} m");
                info[0, 9] = (IUIListItem)new UIListTextValue<string>("Min Rx Power", () => $"{minReceivablePower * Math.Pow(10, 19):n2} E-19 W");
                info[0, 10] = (IUIListItem)new UIListTextValue<string>("Max Tx Power", () => $"{maxTransmittedPower:n2} W");
                info[1, 0] = (IUIListItem)new UIListButton("Antenna Xml");
            }
            return info;
        }
    }
