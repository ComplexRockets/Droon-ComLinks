    ...
    public class ComLinksManager : MonoBehaviour, IDisplayable
    {
        ...
        private void OnSceneloaded(object sender, SceneEventArgs e)
        {
            if (e.Scene == "Flight")
            {
                ...
                StartCoroutine(DCLUtilities.CallNextFrame(Initialise));
            }
        }
        ...
        {
            if (_flightActive) StartCoroutine(DCLUtilities.CallNextFrame(Refresh));
        }
        ...
        public void FlightStateUpdate()
        {
            ...
            GameObject playerCraft = GameObject.FindWithTag("Player");
            if (playerCraft != null) Debug.Log(playerCraft.name);
            else Debug.Log("player craft null");
        ...
        public IUIListItem[,] GetInfo()
        {
            if (info == null)
            {
                ...
                info[0, 0] = (IUIListItem)new UIListTextValue<int>("Network Nodes", () => networkNodes.Count);
                info[0, 1] = (IUIListItem)new UIListTextValue<int>("Network Connections", () => networkConnections.Count);
                info[1, 0] = _player != null ? (IUIListItem)new UIListLabelButton<string>("Player", () => _player.id) as IUIListItem : (IUIListItem)new UIListTextValue<string>("Player", () => _playerCraftNode.Name);
                info[1, 1] = (IUIListItem)new UIListTextValue<bool>("Has Antennas", () => _player != null);
                info[1, 2] = (IUIListItem)new UIListTextValue<bool>("Has Remote Control", () => playerHasRemoteControl);
                info[1, 3] = (IUIListItem)new UIListTextValue<bool>("Has Droon Control", () => playerHasRemoteControl);
                info[3, 0] = (IUIListItem)new UIListTextValue<string>("Refresh Rate", () => $"{(1 / updatePeriod):n2} Hz");
                info[3, 1] = (IUIListItem)new UIListTextValue<int>("Average Fps", () => averageFps);
            }
            return info;
        }
    }
