...
namespace Assets.Scripts.DroonComLinks
{
    public class ComLinksManager : MonoBehaviour {
            ...
            public float updateFrequency => Mod.Instance.updatefrenquency;
                    ...
                    if (_drawMode == "All") {
                        ...
                        foreach (NetworkNode node in _pathsToGS.Keys) {
                            if (_pathsToGS[node].Count != 0) {
                                NetworkConnection connection = ConnectionOf (node, _pathsToGS[node].First () == node ? _pathsToGS[node][1] : _pathsToGS[node].First ());
                                if (connection != null) {
                                    connection.UpdateConnection ();
                                    notUpdatedConnections.Remove (connection);
                                }
            ...
            private void UpdatePathsToGS (List<NetworkNode> nodes) {
                ...
                foreach (NetworkNode node in nodes) {
                    if (node.isCraft) {
                        ...
                        if (list == null) paths.Add (node, GetPathToGS (node));
                        else {
                            List<NetworkNode> newList = list;
                            newList.RemoveRange (0, list.IndexOf (node));
            ...
            private List<NetworkNode> GetPathToGS (NetworkNode initialNode) {
                ...
                bool gsExists = false;
                foreach (NetworkNode node in _nodesInRelayRange) {
                    if (!node.isCraft) {
                        gsExists = true;
                        break;
                    }
                }
                //Debug.Log ("GS: " + gsExists);
                if (!gsExists) return new List<NetworkNode> ();
                ...
                while (_nodesToCheck.Count > 0) {
                    NetworkNode currentNode = _nodesToCheck.First ();
                ...
                NetworkNode closestGS = Array.FindAll (sorted, node => !node.isCraft).First ();
                    ...
                    foreach (StructureNodeData structure in planet.PlanetData.StructureNodes) {
                        try {
                            ...
                            if (structure.PrefabPath.Contains ("PrimaryLaunchSite") || structure.PrefabPath.Contains ("SatelliteDish")) {
                                ...
                                Debug.Log ("ADDED GS at height " + structure.Elevation + 10 + " prefab " + structure.PrefabPath);
                            }
