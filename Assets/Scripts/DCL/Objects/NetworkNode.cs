    ...
    public class NetworkNode : IDisplayable
    {
        ...
        public IUIListItem[,] GetInfo()
        {
            if (info == null)
            {
                ...
                if (isCraft)
                {
                    info[0, 0] = new UIListTextValue<string>("Node Type", () => "Player");
                    info[0, 2] = new UIListTextValue<bool>("Ground Station In Range", () => gsInRange);
                    info[0, 3] = new UIListTextValue<int>("Relay Count", () => relayCount);
                }
                else
                {
                    info[0, 0] = new UIListTextValue<string>("Node Type", () => "Ground Station");
                }
                info[0, 1] = new UIListTextValue<string>("location", () => _parent.Name);
                info[0, 4] = new UIListTextValue<int>("Nodes In Range:", () => stringNodesInRelayRange.Count);
                info[0, 5] = new UIListDropDown(UIListItems.nodesInRelayRangeDropDownID, () => stringNodesInRelayRange);
                info[0, 6] = new UIListTextValue<int>("Nodes In Direct Range:", () => nodesInDirectRange.Count);
                info[0, 7] = new UIListDropDown(UIListItems.nodesInDirectRangeDropDownID, () => inrangeNodeFromId.Keys);
                info[0, 8] = new UIListTextValue<int>("Antennas:", () => antennas.Count);
                info[0, 9] = new UIListDropDown(UIListItems.antennasDropDownID, () => antennaFromId.Keys);
                info[1, 0] = new UIListTextValue<bool>("UnderWater", () => underWater);
            }
            return info;
        }
    }
