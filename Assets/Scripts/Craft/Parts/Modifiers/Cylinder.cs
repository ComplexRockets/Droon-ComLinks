...
public class Cylinder : MonoBehaviour {
    ...
    public void Initialise (bool square, bool highRes, bool a, bool b, Transform parent, IPartScript partScript) {
        ...
        if (square) _cylinder = UnityEngine.Object.Instantiate (_mod.ResourceLoader.LoadAsset<GameObject> (_path + "SquareCylinder.prefab"));
        else if (highRes) _cylinder = UnityEngine.Object.Instantiate (_mod.ResourceLoader.LoadAsset<GameObject> (_path + "HighResCylinder.prefab"));
        else _cylinder = UnityEngine.Object.Instantiate (_mod.ResourceLoader.LoadAsset<GameObject> (_path + "RoundCylinder.prefab"));

        //Debug.Log ("GameObject2 : " + gameObject.name);
        transform.SetParent (parent, worldPositionStays : false);
        _cylinder.transform.SetParent (transform, worldPositionStays : false);
        _partScript.PartMaterialScript.AddRenderer (_cylinder.GetComponentInChildren<Renderer> (), excludeFromDrag : true);

        GameObject Cap;
        if (square) Cap = _mod.ResourceLoader.LoadAsset<GameObject> (_path + "SquareCap.prefab");
        else Cap = _mod.ResourceLoader.LoadAsset<GameObject> (_path + "RoundCap.prefab");
        ...
        if (A) {
            ...
            _partScript.PartMaterialScript.AddRenderer (_sphereA.GetComponentInChildren<Renderer> (), excludeFromDrag : true);
        }
        if (B) {
            ...
            _partScript.PartMaterialScript.AddRenderer (_sphereB.GetComponentInChildren<Renderer> (), excludeFromDrag : true);
        }
    }
